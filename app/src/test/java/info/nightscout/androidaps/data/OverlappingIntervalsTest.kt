...
class OverlappingIntervalsTest {
    ...
    private var list = OverlappingIntervals<TempTarget>()
    @Test fun doTests() {
        // create one 10h interval and test value in and out
        list.add(TempTarget().date(startDate).duration(T.hours(10).mins()).low(100.0).high(100.0))
        ...
        list.add(TempTarget().date(startDate + T.hours(3).msecs()).duration(T.hours(1).mins()).low(200.0).high(200.0))
        Assert.assertEquals(null, list.getValueByInterval(startDate - T.secs(1).msecs()))
        Assert.assertEquals(100.0, list.getValueByInterval(startDate)!!.target(), 0.01)
        Assert.assertEquals(100.0, list.getValueByInterval(startDate + T.hours(5).msecs() - 1)!!.target(), 0.01)
        Assert.assertEquals(null, list.getValueByInterval(startDate + T.hours(5).msecs() + 1))
    ...
    @Test fun testReversingArrays() {
        ...
        someList.add(TempTarget().date(startDate).duration(T.hours(3).mins()).low(200.0).high(200.0))
        someList.add(TempTarget().date(startDate + T.hours(1).msecs()).duration(T.hours(1).mins()).low(100.0).high(100.0))
        list.reset()
        list.add(someList)
